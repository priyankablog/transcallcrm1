...

{
    ...
    public function change_lang_rate($data)
    {

        $select=$this->db->select("*")->where('item_id',$data['item_id'])->where('company_id',$data['company_id'])->get(db_prefix() . 'item_client')->result_array();
        
        if(count($select) > 0){

            $this->db->where('id', $select[0]['id']);
            $this->db->update(db_prefix() . 'item_client',$data);
        }else{
            
            $this->db->insert(db_prefix() . 'item_client', array_merge($data, [
            'dateadded' => date('Y-m-d H:i:s'),
            'addedby'   => is_staff_logged_in() ? get_staff_user_id() : 0,
            ]));
        }
        
        $isStaff = get_staff_user_id();

        log_activity('Language Rate Updated for the Customer [' . $data['company_id'] . ']', $isStaff);

        return $data['company_id'];

    }

    public function get_language_rate($id){
        $data=$this->db->select("*")
            ->where('company_id',$data['company_id'])
            ->get(db_prefix() . 'item_client')
            ->result_array();
        return $data;
    }
