...

{
...

    {
        ...
        $data['copm_master'] = $this->clients_model->get();
...

    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                if (isset($data['wire_transfer']) && $data['wire_transfer']=="on"){

                    unset($data['wire_transfer']);

                    $data['wire_transfer'] = 1;
                    $data['unique_id'] = str_pad(mt_rand(1,99999999),8,'0',STR_PAD_LEFT);

                }else{
                    unset($data['wire_transfer']);
                    $data['wire_transfer'] = 0;
                }
                ...
                $data = $this->input->post();
                if ($data['wire_transfer']=="on") {

                    unset($data['wire_transfer']);
                    $data['wire_transfer'] = 1;
                    if($data['unique_id']==null || $data['unique_id']==""){
                        $data['unique_id'] = str_pad(mt_rand(1,99999999),8,'0',STR_PAD_LEFT);
                    }
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                $data['languages'] =$this->invoice_items_model->get_language(null,$id);
    ...
    public function change_lang_rate($id=''){
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            ...
            foreach($ids as $key => $new_id){
                ...
                $client= $this->clients_model->change_lang_rate($data);
            }
        }

        set_alert('success', _l('rate updated'));
        redirect(admin_url('clients/client/' . $id . '?group=profile'));
    ...
    public function get_client_calls_to_invoice($client_id)
    {
        if (staff_can('create', 'invoices')) {
            $CI = &get_instance();
            $CI->db->select('unique_id')->from(db_prefix() . 'clients')->where('userid', $client_id);
            $unique_id = $CI->db->get()->row()->unique_id;
            $where = [
                'customerid' => $unique_id,
            ];
            $this->load->model('call_summary_model');
            $data['callsdata'] = $this->call_summary_model->get_call_summary('',$where);
            // print_r($callsdata);
            $this->load->view('admin/invoices/invoice_calls_popup_modal', $data);
        // die;
            // $data['billable_tasks'] = $this->projects_model->get_tasks($client_id, [
            //     'billable'     => 1,
            //     'billed'       => 0,
            //     'startdate <=' => date('Y-m-d'),
            // ]);

            // $data['not_billable_tasks'] = $this->projects_model->get_tasks($client_id, [
            //     'billable'    => 1,
            //     'billed'      => 0,
            //     'startdate >' => date('Y-m-d'),
            // ]);
            ...
            // $data['billing_type'] = get_project_billing_type($client_id);
