...

{
    ...
    public function get_client_items(){
        $id = $this->input->post("clientid");
        if(!$id){
            die();
        }
        $this->db->select('tblitems.*,tblitem_client.price,tblitem_client.company_id');
        $this->db->join('tblitem_client','tblitem_client.item_id = tblitems.id and tblitem_client.company_id ='.$id,'left');
        $this->db->order_by("tblitems.id", "desc");
        $data = $this->db->get(db_prefix() . 'items')->result_array();
        $options_html = "<option value=''></option>";
        foreach ($data as $key) {
            if($key['price'] == null || $key['price'] == ""){
                $key['price'] = $key['rate'];
            }

        $options_html .= "<option value='".$key['id']."' data-subtext='".strip_tags(mb_substr($key['long_description'], 0, 200))."'>(".app_format_number($key['price']).') '.$key['description']."</option>";
        }
        echo json_encode(['html'=>$options_html]);
    }
