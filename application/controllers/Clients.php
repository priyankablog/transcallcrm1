...

{
    ...
    public function call_summary($status = false)

    {
        $this->load->model('call_summary_model');
        $where = [

            'customerid' => get_client_unique_id(),

        ];
        $data['cdata'] = $this->call_summary_model->get_call_summary('',$where);

        $data['title']    = "Call Summary";

        $this->data($data);

        $this->view('call_summary');
