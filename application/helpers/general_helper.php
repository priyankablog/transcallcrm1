...
function get_client_unique_id(){
    ...
    $id = get_instance()->session->userdata('client_user_id');
    $CI = &get_instance();
    $CI->db->select('unique_id')->from(db_prefix() . 'clients')->where('userid', $id);
    return $CI->db->get()->row()->unique_id;
}
